...
func wrapBubbleSort(args ...any) any {
	return bubbleSort(intToAny(args...))
}

func wrapBubbleSortOptimalSolution(args ...any) any {
	return bubbleOptimalSolution(intToAny(args...))
}
	...
	for _, test := range tests {
		b.Run(test.name, func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				...
				if test.optimalWay {
					to = wrapBubbleSortOptimalSolution
				} else {
					to = wrapBubbleSort
				}
